			...
			let b = function () {
				var a = setInterval(function() {
                    try{
                        ...
                        $$('#app > div.je-panel.je-admin-layout-main > div > div.je-panel-wrapper > div.je-panel-item.je-panel-item-region-default > div > div > div > div > div > div > div > div.je-panel-wrapper > div > div > div > div > div > div > div > div.je-panel-wrapper > div > div > div > div > div.je-panel-item.je-panel-item-region-default > div > div.vxe-table--render-wrapper > div.vxe-table--fixed-wrapper > div > div.vxe-table--body-wrapper.fixed-right--wrapper > table > tbody > tr > td.vxe-body--column.col_37.col--left.col--action.col--last.col--ellipsis > div > div > div:nth-child(1)').hide()
                    }catch(e){
                        console.error(`${new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString()}: 屏蔽消息弹窗,没找到窗体`)
                    }
					console.warn(`${new Date().toLocaleDateString() + ' ' + new Date().toLocaleTimeString()}: 屏蔽消息弹窗`)
					clearInterval(a);
					b();
				}, 3*1000);
			}
