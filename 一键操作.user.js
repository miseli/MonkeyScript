...
async function doUpdate是否实施(flag=false){
    ...
    let { data:rowdata } = await ajax_.post('http://10.10.15.130/je/sp/ticket/application/load', $qs.stringify({
        ...
        j_query: `{"custom":[{"type":"and","value":[{"code":"TICKET_LEVEL","cn":"and","type":"like","value":"TJDHZY,YJDHZY,EJDHZY"},{"code":"SY_AUDFLAG","cn":"and","type":"like","value":"ENDED"},{"code":"START_TIME","cn":"and","type":"between","value":["${$tool.DateFormat(new Date(), 'yyyy-MM-dd')} 00:00:00","${$tool.DateFormat(new Date(), 'yyyy-MM-dd')} 23:59:59"]}],"cn":"and"}],"order":[],"_types":["group"]}`,
        page:1,
        limit:30,
        start:0
...
if(/10\.10\.54\.18/.test(location.href)){
    ...
    let handler = function(){
        let start_t = `${new Date().toLocaleDateString()} 17:00`
        ,end_t = `${new Date().toLocaleDateString()} 17:05`
        ,now_t = Date.now()

        if(now_t>new Date(start_t).getTime() && now_t<new Date(end_t).getTime()){
        //if(dayjs().isBetween(start_t, end_t)){
            doUpdate是否实施(true);
        }

        if(tid!=0) {
            clearInterval(tid);
            console.log('清除setInterval'+tid)
            tid = 0;
        }
        tid = setInterval(handler,30000)
        console.log('开始setInterval'+tid)
    }
    handler()
