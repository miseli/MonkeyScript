...
// @require      https://cdnjs.cloudflare.com/ajax/libs/dayjs/1.11.23/dayjs.min.js
...
async function change_password(login_name, newpassword = "Aa@123456") {
	...
	// {I:'-3116385039151185264',K:'XXXX人名',N:1105,C:0} 返回失败
...
//10.10.15.130平台自动填已实施
async function doUpdate是否实施(flag=false){
    ...
    //根据条件查询作业
    let { data:rowdata } = await ajax_.post('http://10.10.15.130/je/sp/ticket/application/load', $qs.stringify({
        tableCode:'TICKET_APPLICATION',
        funcCode: 'TICKET_APPLICATION',//SY_AUDFLAG表示审批状态
        j_query: `{"custom":[{"type":"and","value":[{"code":"COMPANY_NAME","cn":"and","type":"like","value":"乙烯分公司"},{"code":"TICKET_LEVEL","cn":"and","type":"like","value":"TJDHZY,YJDHZY,EJDHZY,SXKJZY"},{"code":"SY_AUDFLAG","cn":"and","type":"like","value":"ENDED"},{"code":"START_TIME","cn":"and","type":"between","value":["${$tool.DateFormat(new Date(), 'yyyy-MM-dd')} 00:00:00","${$tool.DateFormat(new Date(), 'yyyy-MM-dd')} 23:59:59"]}],"cn":"and"}],"order":[],"_types":["group"]}`,
        page:1,
        limit:30,
        start:0
    }),{headers:{pd:'security-platform-ticket'}})
    let rows = rowdata.data.rows
    console.log('这里有bug,需要修复')

    let task = []
    //遍历查询到的每个作业
    rows.forEach(async data1 => {
        let {TICKET_APPLICATION_ID} = data1
        // 多层解构
        let {data: {data: data2}} = await ajax_.post('http://10.10.15.130/je/sp/ticket/application/getInfoById', $qs.stringify({
            tableCode: 'TICKET_APPLICATION',
        ...
        const metaInfo = {
            "funcCode": "TICKET_APPLICATION_EDIT",
            "tableCode": "TICKET_APPLICATION",
            "pkCode": "TICKET_APPLICATION_ID",
            "IS_IMPLEMENT_DHZY": flag?"1":"", //已实施//"REMARK": ""
            "codeGenFieldInfo": "[{\"code\":\"CODE\",\"configInfo\":\"[{\\\"qzfs\\\":\\\"BDZD\\\",\\\"dyz\\\":\\\"COMPANY_SIMPLENAME\\\",\\\"gs\\\":\\\"\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":\\\"\\\",\\\"qsh\\\":\\\"\\\",\\\"bc\\\":\\\"\\\",\\\"zq\\\":\\\"\\\",\\\"index\\\":1},{\\\"qzfs\\\":\\\"CL\\\",\\\"dyz\\\":\\\"-\\\",\\\"gs\\\":\\\"\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":\\\"\\\",\\\"qsh\\\":\\\"\\\",\\\"bc\\\":\\\"\\\",\\\"zq\\\":\\\"\\\",\\\"index\\\":2},{\\\"qzfs\\\":\\\"BDZD\\\",\\\"dyz\\\":\\\"DEPT_NAME_PHONETIC\\\",\\\"gs\\\":\\\"\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":\\\"\\\",\\\"qsh\\\":\\\"\\\",\\\"bc\\\":\\\"\\\",\\\"zq\\\":\\\"\\\",\\\"index\\\":3},{\\\"qzfs\\\":\\\"CL\\\",\\\"dyz\\\":\\\"-\\\",\\\"gs\\\":\\\"\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":\\\"\\\",\\\"qsh\\\":\\\"\\\",\\\"bc\\\":\\\"\\\",\\\"zq\\\":\\\"\\\",\\\"index\\\":4},{\\\"qzfs\\\":\\\"QJBL\\\",\\\"dyz\\\":\\\"@NOW_DATE@\\\",\\\"gs\\\":\\\"yyyyMMdd\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":\\\"\\\",\\\"qsh\\\":\\\"\\\",\\\"bc\\\":\\\"\\\",\\\"zq\\\":\\\"\\\",\\\"index\\\":5},{\\\"qzfs\\\":\\\"LSH\\\",\\\"dyz\\\":\\\"TABLE\\\",\\\"gs\\\":\\\"\\\",\\\"jqws\\\":\\\"\\\",\\\"cd\\\":4,\\\"qsh\\\":\\\"0001\\\",\\\"bc\\\":1,\\\"zq\\\":\\\"DAY\\\",\\\"index\\\":7}]\",\"funcId\":\"AmVh8iW845AsaOdEfwc\",\"funcCode\":\"TICKET_APPLICATION_EDIT\",\"funcName\":\"车间报备编辑\",\"tableCode\":\"TICKET_APPLICATION\"}]"
        };

        if(!flag||!data2.IS_IMPLEMENT_DHZY){
            const data3 = {...data2,...metaInfo}
            task.push(ajax_.post('http://10.10.15.130/je/sp/ticket/application/doUpdate', $qs.stringify(data3),{headers:{pd:'security-platform-ticket'}}))
        }else{
            console.log(`${data2.CODE}已经填写`)
        }
    })
    let ret = await Promise.allSettled(task)
    debugger
    console.log(ret)
}

function 自动流程(action_do) {
	// 流程:
	...
	function getUser() {
		//http://10.10.15.130/je/rbac/cloud/account/currentUser也可以get次链接得到当前用户
	...
	async function 处理待办(action = '同意', filters = '报备') {
		...
		rows.forEach(async item => {
			...
			let funcId = title.includes('补报')?'SBmnFCYkttZ0WNtFqnk':'Es3xHzD7rK1r0f2NHm2';
...
async function 解绑定位卡(){
	let { value: ids } = await Swal.fire({
		...
		inputPlaceholder: '卡号BTT310XXXXX\n卡号BTT310XXXXX\n卡号BTT310XXXXX\n...',
		inputAttributes: {
			'aria-label': 'Type your message here'
		},
		showCancelButton: true
	...
	for(let id of ids){
        if(id.length!=0){
            id = '0000000' + id
            id = 'BTT310' + id.substr(-5)
        }
        解绑定位卡_处理过程(id)
	}
}
//解绑过程未提示姓名,待补充
async function 解绑定位卡_处理过程(LABELID = "BTT31007802"){
	let ajax_ = $axios.create({
		// headers:{
		// authorization: localStorage.getItem("authorization"),
		// 'content-type': 'application/x-www-form-urlencoded;charset=UTF-8',
	...
	let postinfo = {
		"一人一档": {
			url: 'http://10.10.15.130/je/sp/base/persionnel/load',
			data: $qs.stringify({
							tableCode:'BASE_USER',
							funcCode: 'BASE_USER',
							j_query:`{"custom":[{"type":"and","value":[{"code":"LABEL","cn":"and","type":"like","value":"${LABELID}"}],"cn":"and"}],"order":[],"_types":["group"]}`,
							page:1,
							limit:30,
							start:0
					}),
			headers: {pd:'security-platform-base'},
			async handler(data1){
				let {JE_RBAC_USER_ID, USER_CODE} = data1
				let ret = await ajax_.post('http://10.10.15.130/je/sp/base/persionnel/unbind',$qs.stringify({JE_RBAC_USER_ID, USER_CODE}),{headers: {pd:'security-platform-base'}})
				console.debug(ret)
				return ret
			}
		},
		"承包商": {
			url: 'http://10.10.15.130/je/contractorPersonnelInfo/load',
			data: $qs.stringify({
							tableCode:'CONTRACTOR_PERSONNEL_INFO',
							funcCode: 'CONTRACTOR_PERSONNEL_INFO',
							j_query:`{"custom":[{"type":"and","value":[{"code":"CARD_NUMBER","cn":"and","type":"like","value":"${LABELID}"}],"cn":"and"}],"order":[],"_types":["group"],"strategyId":"2c821717812e462ab0462ddbeb17ceca"}`,
							page:1,
							limit:30,
							start:0
					}),
			headers: {pd:'security-platform-contractor'},
			async handler(data1){
					let {CONTRACTOR_PERSONNEL_INFO_ID} = data1
					let {data: data2} = await ajax_.post('http://10.10.15.130/je/common/getInfoById', $qs.stringify({
						tableCode: "CONTRACTOR_PERSONNEL_INFO",
						funcCode: "CONTRACTOR_PERSONNEL_INFO",
						pkValue: CONTRACTOR_PERSONNEL_INFO_ID
					}), {headers: {pd:'security-platform-contractor'}})
					data2 = data2.data
					let temp = { ...data1, ...data2 }
					delete temp.ROW_ID;
					temp.CARD_NUMBER = ""
					temp = 清洗(temp)
					temp.BIND_TYPE = "0";
					temp.tableCode = "CONTRACTOR_PERSONNEL_INFO";
					temp.funcCode = "CONTRACTOR_PERSONNEL_INFO";
					temp.pkCode = "CONTRACTOR_PERSONNEL_INFO";
					let data3 = temp

					//解绑请求
					let ret = await ajax_.post('http://10.10.15.130/je/contractorPersonnelCard/doUpdate', $qs.stringify(data3), {headers: {pd:'security-platform-contractor'}})
					console.debug(ret)
					return ret
			}
		},
		"访客": {
			url: 'http://10.10.15.130/je/contractorVisitorInfo/load',
			data: $qs.stringify({
							tableCode:'CONTRACTOR_VISITOR_INFO',
							funcCode: 'CONTRACTOR_VISITOR_INFO',
							j_query:`{"custom":[{"type":"and","value":[{"code":"CARD_NUMBER","cn":"and","type":"like","value":"${LABELID}"}],"cn":"and"}],"order":[],"_types":["group"]}`,
							page:1,
							limit:30,
							start:0
					}),
			headers: {pd:'security-platform-contractor'},
			handler: async function(data1){
					let {CONTRACTOR_VISITOR_INFO_ID} = data1
					let {data: data2} = await ajax_.post('http://10.10.15.130/je/common/getInfoById', $qs.stringify({
						tableCode: "CONTRACTOR_VISITOR_INFO",
						funcCode: "CONTRACTOR_VISITOR_INFO",
						pkValue: CONTRACTOR_VISITOR_INFO_ID
					}), {headers: {pd:'security-platform-contractor'}})
					data2 = data2.data
					let temp = { ...data1, ...data2 }
					delete temp.ROW_ID;
					...
					temp = 清洗(temp)
					temp.BIND_TYPE = "0";
					temp.tableCode = "CONTRACTOR_VISITOR_INFO";
					temp.funcCode = "CONTRACTOR_VISITOR_INFO";
					temp.pkCode = "CONTRACTOR_VISITOR_INFO_ID";
					let data3 = temp

					//解绑请求
					let ret = await ajax_.post('http://10.10.15.130/je/contractorVisitorCard/doUpdate', $qs.stringify(data3), {headers: {pd:'security-platform-contractor'}})
					console.debug(ret)
					return ret
			}
		},
...
/*     async function search_func(){
        let fetchDatas = []
        for(let [k,v] of Object.entries(postinfo)){
            fetchDatas.push(ajax_.post(v.url, v.data, {headers: v.headers}).then(({data})=>{
                data = data.data.rows
                return data.length==0?Promise.reject('0'):data;
            }))
        }
        return Promise.any(fetchDatas).catch(e=>[])
    }
    let a = await search_func();
    debugger
    return; */

    Object.entries(postinfo).forEach(async ([k,v])=>{
        //搜索数据
        let {data: rowdata} = await ajax_.post(v.url, v.data, {headers: v.headers})
        let rows = rowdata.data.rows
        if(rows&&rows.length==0)return;

        //解绑数据
        rows.forEach(async data1 => {
            console.log(`${k}${data1.REALNAME}搜索数据:`, data1)
            let {data} = await v.handler(data1)

            console.log(`${k}${data1.REALNAME}解绑数据:`, data)

            if(data.code=='9999'){
                $swal.fire(data.message)
            }else if(data.code=='1000'){
                $swal.fire(`${k}${data1.REALNAME}解绑${LABELID}: ${data.message}`)
                //return {REALNAME:data1.REALNAME, LABELID, message: data.message}
            }else{
                console.log(data)
            }
            console.log(`${k}${data1.REALNAME}解绑${LABELID}:`, data.message)
        })
    })
...
if(/10\.10\.54\.18/.test(location.href)){
	...
	//批量修改密码()
	//打分()
    const menu_command_id_1 = GM_registerMenuCommand("只填内容", async function(e) {
        alert("只填内容");
        await updataScore(6)
    }, {
        accessKey: "a",
    ...
    const menu_command_id_2 = GM_registerMenuCommand("打分", async function(e) {
        alert("打分");
        await updataScore(1)
    }, {
        accessKey: "b",
    ...
    (()=>{
        ...
        let handler = function(){
            let start_t = `${new Date().toLocaleDateString()} 15:50`
            ,end_t = `${new Date().toLocaleDateString()} 17:00`
            ,now_t = Date.now()

            if(now_t>new Date(start_t).getTime() && now_t<new Date(end_t).getTime()){
                doUpdate是否实施(true);
            }

            if(tid!=0) {
                clearInterval(tid);
                console.log('%c自动流程清除setInterval'+tid, 'color: red; font-weight: bold;')
                tid = 0;
            }
            tid = setInterval(handler,30000)
            console.log('%c自动流程开始setInterval'+tid, 'color: red; font-weight: bold;')
        }
        handler()
    })();
    ...
    const menu_command_id_2 = GM_registerMenuCommand("自动实施", async function(e) {
        await doUpdate是否实施(true)
    }, {
        accessKey: "b",
        autoClose: true
    });

    const menu_command_id_3 = GM_registerMenuCommand("删除实施", async function(e) {
        await doUpdate是否实施(false)
    }, {
        accessKey: "c",
        autoClose: true
    });
